...
public class VatPhamDAO {
    ...
    public List<VatPham> searchByID(String id){
        String sql = "SELECT VP.*,L.TenLoai FROM VatPham AS VP INNER JOIN Loai AS L ON VP.MaLoai = L.MaLoai WHERE MaVP LIKE '%"+ id +"%'";
        return getData(sql);
    }

    public List<VatPham> searchByName(String name){
        String sql = "SELECT VP.*,L.TenLoai FROM VatPham AS VP INNER JOIN Loai AS L ON VP.MaLoai = L.MaLoai WHERE TenVP LIKE '%"+ name +"%'";
        return getData(sql);
    }

    public List<VatPham> searchByDes(String des){
        String sql = "SELECT VP.*,L.TenLoai FROM VatPham AS VP INNER JOIN Loai AS L ON VP.MaLoai = L.MaLoai WHERE MoTa LIKE '%"+ des +"%'";
        return getData(sql);
    }

    public List<VatPham> searchByType(String type){
        String sql = "SELECT VP.*,L.TenLoai FROM VatPham AS VP INNER JOIN Loai AS L ON VP.MaLoai = L.MaLoai WHERE MaLoai LIKE '%"+ type +"%'";
        return getData(sql);
    }

    public List<VatPham> searchByStaff(String staff){
        String sql = "SELECT VP.*,L.TenLoai FROM VatPham AS VP INNER JOIN Loai AS L ON VP.MaLoai = L.MaLoai WHERE MaNV LIKE '%"+ staff +"%'";
        return getData(sql);
    }
}
