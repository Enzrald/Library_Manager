...
public class ThongKeDAO {
    ...
    public int getVon(String tuNgay, String denNgay){
        ...
        String sqlDoanhThu = "SELECT SUM(Von) AS TongVon FROM Phieu WHERE substr(NgayThang,1,4)||substr(NgayThang,6,2)||substr(NgayThang,9,2) BETWEEN ? AND ?";
        List<Integer> list = new ArrayList<>();
        Cursor cursor = db.rawQuery(sqlDoanhThu, new String[]{tuNgay,denNgay});

        while (cursor.moveToNext()){
            try {
                list.add(Integer.parseInt(cursor.getString(cursor.getColumnIndexOrThrow("TongVon"))));
            }catch (Exception e){
                list.add(0);
            }
        }
    ...
    public int getLai(String tuNgay, String denNgay){
        tuNgay = tuNgay.replace("/","");
        denNgay = denNgay.replace("/","");
        String sqlDoanhThu = "SELECT SUM(Lai) AS TongLai FROM Phieu WHERE substr(NgayThang,1,4)||substr(NgayThang,6,2)||substr(NgayThang,9,2) BETWEEN ? AND ?";
        List<Integer> list = new ArrayList<>();
        Cursor cursor = db.rawQuery(sqlDoanhThu, new String[]{tuNgay,denNgay});

        while (cursor.moveToNext()){
            try {
                list.add(Integer.parseInt(cursor.getString(cursor.getColumnIndexOrThrow("TongLai"))));
            }catch (Exception e){
                list.add(0);
            }
        }
